...
function fieldIcon(field: SchemaField) {
  ...
  if (key.includes("sku")) return <Hash size={14} className="text-[#F8BC42]" />;
  if (key.includes("quantity") || key.includes("stock")) return <Layers size={14} className="text-[#F8BC42]" />;
  if (key.includes("supplier")) return <Truck size={14} className="text-[#667085]" />;
  if (key.includes("name") || key.includes("product")) return <Package size={14} className="text-[#F8BC42]" />;
  return null;
}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search…"
            className="h-9 w-36 rounded-lg border border-[#E7EAF5] bg-[#FAFBFF] pl-8 pr-2 text-xs text-[#1A1F36] outline-none focus:border-[#F8BC42]"
          />
        </div>
        <button
          type="button"
  ...
  const renderMetric = (comp: SchemaComponent) => (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {comp.fields.map((f, i) => {
        ...
        const accents = ["#F8BC42", "#FF5996", "#FF5996"];
        return (
          <div
            key={f.id}
          ...
          >
            ...
            <span className="mt-1 block text-xs font-medium" style={{ color: accents[i] ?? "#F8BC42" }}>
        ...
        {isInventory
          ? [
              ...
              <div key={bar.label} className="flex h-full flex-1 flex-col items-center justify-end gap-2">
                <div
                  style={{ height: `${bar.h}%` }}
                  className="w-full rounded-t bg-gradient-to-t from-[#F8BC42] to-[#E0A22B] opacity-90 transition-all hover:opacity-100"
                />
          ...
          : [40, 75, 55, 90, 60, 85].map((h, i) => (
              <div key={i} className="flex h-full flex-1 flex-col items-center justify-end gap-2">
                <div
                  style={{ height: `${h}%` }}
                  className="w-full rounded-t bg-gradient-to-t from-[#F8BC42] to-[#E0A22B]"
                />
    ...
    <ul className="divide-y divide-[#E7EAF5] overflow-hidden rounded-2xl border border-[#E7EAF5] bg-white">
      {[0, 1, 2].map((i) => (
        <li key={i} className="flex items-center gap-3 p-4 transition-colors hover:bg-[#F4F5FF]">
          <div className="h-2 w-2 shrink-0 rounded-full bg-[#F8BC42]" />
