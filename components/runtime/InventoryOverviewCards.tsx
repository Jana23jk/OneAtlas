...
function AnimatedStatCard({ stat, delay }: { stat: OverviewStat; delay: number }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-4 text-3xl font-bold tabular-nums text-[#0A2540]">
        {animated.toLocaleString()}
        {stat.suffix ? (
          <span className="ml-1 text-lg font-semibold text-[#667085]">{stat.suffix}</span>
        ) : null}
      </p>
      <p className="mt-2 text-xs font-medium text-[#00D4B1]">↑ Updated live from runtime</p>
    </div>
  );
}
