      ...
      <div className="flex divide-x divide-white/[0.06] text-left">
        ...
        <div className="hidden w-44 flex-col gap-1.5 p-3 sm:flex bg-[#071D33]/20">
          {["Contacts", "Deals", "Pipeline", "Reports"].map((item, i) => (
            <div key={item}
              className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${i === 0 ? "bg-[#635BFF]/15 text-[#7A73FF]" : "text-[#8892A4] hover:text-white/60"}`}>
              {item}
            </div>
          ))}
        </div>
        {/* Main panel */}
        <div className="flex-1 p-4 bg-[#0A2540]">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-xs font-semibold text-white/90">Contacts</span>
            <span className="rounded bg-[#635BFF]/20 px-2 py-0.5 text-[10px] text-[#8a84ff] font-medium">+ Add field</span>
          </div>
  ...
  return (
    <section className="relative overflow-hidden pt-[68px]">
      <div className="relative mx-auto max-w-5xl px-6 pb-12 pt-20 text-center">
        ...
        {/* Headline */}
        <h1 className="animate-fade-in-up mx-auto max-w-4xl text-5xl font-extrabold leading-[1.1] tracking-tight text-[#0A2540] sm:text-6xl lg:text-7xl">
