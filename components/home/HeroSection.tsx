...
function BuilderMock() {
  ...
  return (
    <div className="animate-float relative mx-auto mt-16 max-w-3xl overflow-hidden rounded-2xl border border-[#7A73FF]/15 shadow-2xl shadow-[#7A73FF]/8"
      style={{ background: "#1A1F36" }}>
      {/* Window chrome */}
      <div className="flex items-center gap-2 border-b border-white/[0.08] px-4 py-3 bg-[#071D33]/40">
        <span className="h-2.5 w-2.5 rounded-full bg-[#FFB17A]" />
        <span className="h-2.5 w-2.5 rounded-full bg-[#FFB17A]" />
        <span className="h-2.5 w-2.5 rounded-full bg-[#7A73FF]" />
      ...
      <div className="flex divide-x divide-white/[0.06] text-left">
        ...
        <div className="hidden w-44 flex-col gap-1.5 p-3 sm:flex bg-[#071D33]/20">
          {["Contacts", "Deals", "Pipeline", "Reports"].map((item, i) => (
            <div key={item}
              className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${i === 0 ? "bg-[#7A73FF]/15 text-[#7A73FF]" : "text-[#8892A4] hover:text-white/60"}`}>
              {item}
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 p-4 bg-[#1A1F36]">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="rounded bg-[#7A73FF]/20 px-2 py-0.5 text-[10px] text-[#8a84ff] font-medium">+ Add field</span>
          </div>
              ...
              <div key={i} className="grid grid-cols-4 border-b border-white/[0.04] px-3 py-2 last:border-0 hover:bg-white/[0.01]">
                {row.map((cell, j) => (
                  <span key={j} className={`text-xs ${j === 2
                    ? cell === "Active" ? "text-[#7A73FF]" : cell === "Lead" ? "text-[#FFB17A]" : "text-[#FFB17A]"
      ...
      <div className="border-t border-white/[0.08] px-4 py-3 bg-[#071D33]/40">
        <div className="flex items-center gap-2 rounded-lg border border-[#7A73FF]/20 bg-[#7A73FF]/5 px-3 py-2">
          <span className="text-xs text-[#7A73FF]">✦</span>
          <span className="text-xs text-[#8892A4]">add a revenue field to Contacts…</span>
          <span className="ml-auto h-3.5 w-0.5 animate-pulse bg-[#7A73FF]" />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <section className="relative overflow-hidden pt-[68px]">
      <div className="relative mx-auto max-w-5xl px-6 pb-12 pt-20 text-center">
        {/* Badge */}
        <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#7A73FF]/20 bg-[#7A73FF]/5 px-4 py-1.5">
          <span className="h-1.5 w-1.5 rounded-full bg-[#7A73FF] shadow-sm shadow-[#7A73FF]" />
          <span className="text-xs font-semibold text-[#7A73FF]">Now in public beta — schema versioning live</span>
        </div>
        ...
        <h1 className="animate-fade-in-up mx-auto max-w-4xl text-5xl font-extrabold leading-[1.1] tracking-tight text-[#1A1F36] sm:text-6xl lg:text-7xl">
          ...
          <span className="bg-gradient-to-r from-[#7A73FF] to-[#FFB17A] bg-clip-text text-transparent">
            thought.
          </span>
        </h1>
        ...
        <form onSubmit={handleGenerate} className="animate-fade-in-up mx-auto mt-10 max-w-2xl w-full" style={{ animationDelay: "0.15s" }}>
          <div className="relative flex items-center p-2 rounded-2xl bg-white border border-[#7A73FF]/10 shadow-lg shadow-[#7A73FF]/5 hover:border-[#7A73FF]/30 transition-all duration-300 focus-within:border-[#7A73FF] focus-within:ring-4 focus-within:ring-[#7A73FF]/15">
            <input
              type="text"
              value={prompt}
            ...
            <button
              type="submit"
              className="shrink-0 flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-[#7A73FF] to-[#6B64E8] px-6 py-3 text-sm font-semibold text-white shadow-md shadow-[#7A73FF]/20 hover:opacity-95 transition-all duration-300 hover:-translate-y-[3px] hover:shadow-lg hover:shadow-[#7A73FF]/30 active:scale-[0.98] cursor-pointer"
            >
              <span>Generate</span>
              <ArrowRight size={15} />
            </button>
        ...
        <div className="animate-fade-in-up mt-5 flex flex-wrap items-center justify-center gap-2.5 max-w-2xl mx-auto" style={{ animationDelay: "0.2s" }}>
          ...
          {chips.map((chipText) => (
            <button
              ...
              className="rounded-full bg-white border border-[#7A73FF]/10 px-4 py-1.5 text-xs font-medium text-[#475467] shadow-sm hover:bg-[#F4F5FF] hover:border-[#7A73FF] hover:text-[#7A73FF] hover:-translate-y-0.5 transition-all duration-200 cursor-pointer"
            >
              {chipText}
            </button>
          ))}
        ...
        <div className="animate-fade-in-up mt-10 flex flex-wrap items-center justify-center gap-6"
          ...
          {trustBadges.map((badge) => (
            <span key={badge.label}
              ...
              <span className="text-[#7A73FF]">{badge.icon}</span>
              {badge.label}
            </span>
          ))}
        </div>
