      ...
      <div className="mx-auto max-w-7xl space-y-10 px-6 py-8 pb-16 animate-fade-in-up">
        ...
        <header className="reveal flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-3">
            <Link
              href={`/builder/${appId}`}
              className="inline-flex items-center gap-2 text-sm font-medium text-[#667085] transition-colors hover:text-[#F8BC42]"
            >
              <ArrowLeft size={16} />
              Back to builder
            </Link>
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-[#F8BC42]">
                Deployment center
              </p>
              ...
              {ENVIRONMENTS.map((env) => (
                <button
                  ...
                  className={`rounded-lg px-3 py-1.5 text-xs font-semibold capitalize transition-all ${
                    environment === env
                      ? "bg-gradient-to-r from-[#F8BC42] to-[#E0A22B] text-white shadow-md"
        ...
        <div className="reveal reveal-delay-1 dashboard-card flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <span
              ...
              style={{ backgroundColor: deploying ? "#F8BC42" : "#00D4B1" }}
            />
            <div>
          ...
          <div className="flex items-center gap-3">
            <Link
              href={`/run/${appId}`}
              target="_blank"
              className="inline-flex items-center gap-1.5 rounded-lg border border-[#E7EAF5] bg-[#FAFBFF] px-3 py-2 text-xs font-semibold text-[#1A1F36] transition-colors hover:border-[#F8BC42]/30"
            >
              <Globe size={14} />
              Open live app
            </Link>
            <span
              ...
              style={{
                backgroundColor: latestStatus === "success" ? "rgba(0, 212, 177, 0.12)" : "rgba(248, 188, 66, 0.12)",
                color: latestStatus === "success" ? "#00D4B1" : "#F8BC42",
              }}
            >
              {latestStatus}
            </span>
          ...
          {deploying ? (
            <div className="w-full">
              <div className="h-2 overflow-hidden rounded-full bg-[#E7EAF5]">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-[#F8BC42] to-[#E0A22B] transition-all duration-500"
